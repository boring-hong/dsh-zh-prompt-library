...
同一套检索能力另有一个独立 MCP server：`github.com/boring-hong/zh-prompt-search-mcp`
（当前 `v0.1.1`）。给用不了 DSH 插件的场景（Claude Code / Codex / Cursor…）。

两份副本的边界：

| | 插件仓库 | MCP 仓库 |
|---|---|---|
| 检索模块 | `lib/zhretriever.js` | `zhretriever.js`（vendored） |
| 语料 | `lib/zh-corpus.json` | `zh-corpus.json`（vendored） |
| 谁改 | 改这里，然后同步出去 | 不要手改，改完在插件侧同步回来 |

`node sync-corpus.mjs --local <插件目录>` 负责同步，两条保护：
比对前会剥离 vendor 头（否则每次运行都重复追加头部而误报"有改动"）；
目标有本地改动时**拒绝覆盖**，必须显式 `--force`，避免把本地修复静默退回旧版。
...
| 版本 | 语料 | 检索逻辑 |
|---|---|---|
| `0.2.0` | 中文：379 条 general + 24 类骨架；英文：prompts.chat 2,306 条 | 初版 |
| `0.2.1` | 同上 | 包名统一为 `dsh-zh-prompt-library` |
| `0.2.2` | 同上 | 见下：短查询漏召回 / 静默丢弃文档 / 无上限注入 |

### 0.2.2 改了检索门槛（三处，都是实测撞出来的）

改的是 `lib/zhretriever.js`，DSH 插件与 MCP 仓库（`zh-prompt-search-mcp`）共用同一份，
改完要 `node sync-corpus.mjs` 同步，否则两份会漂移。

1. **静默丢弃**：原本要求文档中文 ≥500 字才索引。拿本机 `~/.dsh/skills` 实测，
   30 个文档里 21 个被丢掉 —— 写得好但短的风格/工作流文档全都没进检索。阈值降到 **150**，
   并在 `ZH_PROMPT_DEBUG=1` 时点名列出被跳过的文件与实际阈值。
   实测效果：可检索文档从 9 个涨到 **29 个**。
2. **短查询漏召回**：CJK 2-gram 让「膜清洗」产出 `膜清/清洗/洗方` 三个重叠二字组，
   而文档通常只含其中两个，于是「写一个 MBR 膜清洗方案」只命中 3 词，卡在固定的 `>=4`
   门槛上 —— 恰好漏掉最该命中的 MBR 文档。改成随查询长度缩放：
   `minMatch = min(4, max(2, ceil(词数/2)))`。实测「写一个 MBR 膜清洗方案」现在能命中
   `boujoy-expert-waterplant.md`，且「今天天气怎么样 / 写一首古诗 / 翻译这段话」仍不误召回。
3. **无上限注入**：命中专家文档后整篇注入，单篇超大文档会挤占上下文。新增
   `ZH_PROMPT_SKILLS_MAX_CHARS`（默认 8000）截断并标注；插件侧另有 `clip(body, 6000)` 兜底。

`stats.skills` 的语义也一并修正：原来报「磁盘上有几篇」，会出现「本地专家文档 1 个」却
一条专家片段都没有的自相矛盾；现在报**本次实际命中数**，原信息保留在 `skillsLoaded`。
