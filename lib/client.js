/**
 * dsh-zh-prompt-library 浏览器端 bundle（单文件，__ModuleLoader__ 加载）
 *
...
window.__ModuleLoader__.load({
  ...
  factory: (require) => {
    ...
    const ENHANCE_URL = '/dsh-zh-prompt-library/enhance'
...
.enh-st { font-size:11px; white-space:nowrap; font-weight:500; color:var(--dsw-alias-label-tertiary);
  ...
  animation: enh-st-in .18s ease-out; }
/* 自动收起的提示淡出，避免 3 秒后"啪"地消失；常驻提示不会被移除，所以这条只影响前者。 */
.enh-st[data-auto="1"] { animation: enh-st-in .18s ease-out, enh-st-out .3s ease-in 2.7s forwards; }
@keyframes enh-st-in  { from { opacity:0; transform:translateX(-3px); } to { opacity:1; transform:none; } }
@keyframes enh-st-out { to { opacity:0; } }
...
@media (prefers-reduced-motion: reduce) {
  ...
  .enh-btn, .enh-st { transition: none !important; animation: none !important; }
}
`
    ...
    function Button(props) {
      ...
      const [skin, setSkin] = React.useState(() => {
        try {
          const saved = window.localStorage.getItem('dsh-zh-prompt-library-skin')
      ...
      const cycleSkin = () => {
        ...
        setSkin(next)
        try {
          window.localStorage.setItem('dsh-zh-prompt-library-skin', next)
        } catch (error) {
          /* 无 localStorage 也不影响本次会话内切换 */
        }
        const name = { green: '绿（流萤）', blue: '蓝', pink: '粉' }[next] || next
        // 每次切换都换一个新对象（见下方 useEffect 的依赖说明）。
        setNote({ kind: 'ok', text: '皮肤：' + name + '（再右键继续切换）', auto: true })
      }

      // 提示自动消失。
      //
      // 这里要区分「看一眼就够」和「需要停留」两类：
      //   auto:true（切皮肤 / 已撤销 / 检索词 / 改写耗时）—— 3 秒后自动收起；
      //   auto 缺省 + kind:'err'（失败原因、自检信息）—— 一直留着。
      //     失败提示会自动消失是排障的灾难：你得再撞一次同样的错才能再看一眼。
      //   «检索并改写中…» 由 busy 控制，不参与计时，否则计时器会在请求还在跑时把它抹掉。
      // 定时器在每次 note 变化时都被 cleanup 清掉，所以旧提示的计时不会误伤新提示。
      React.useEffect(() => {
        if (!note || !note.auto) return undefined
      ...
      const run = () => {
        ...
        if (undoRef.current) {
          ...
          undoRef.current = ''
          setNote({ kind: 'ok', text: '已撤销', auto: true })
          return
        }
        const text = draft
        if (!text || !text.trim()) {
        ...
        postJson(ENHANCE_URL, { text }, 120000)
          .then(({ status, text: body }) => {
            ...
            setNote({ kind: 'ok', text: parsed.ms + 'ms · 模板' + parsed.usedTemplates + ' · ' + route + skip + kw, auto: true })
          })
          .catch((error) => {
            setBusy(false)
      ...
      return React.createElement(
        ...
        React.createElement(
          'button',
          {
            ...
            onContextMenu: (event) => {
              ...
              if (kwRef.current) {
                setNote({ kind: 'ok', text: '检索词: ' + kwRef.current, auto: true })
                kwRef.current = ''
                return
              }
              cycleSkin()
        ...
        note
          ? React.createElement(
              'span',
              {
                className: 'enh-st enh-skin-' + skin,
                'data-kind': note.kind,
                // 标记「会自己消失」的提示，供 CSS 只在它们身上挂淡出动画。
                'data-auto': note.auto ? '1' : '0',
              },
              note.text,
            )
          : null,
